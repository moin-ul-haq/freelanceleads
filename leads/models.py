...
class Lead(models.Model):
    ...
    audit_done  = models.BooleanField(default=False)  # True when Celery audit task completes
    ...
    class Meta:
        ...
        indexes  = [
            models.Index(fields=['niche', 'city', 'state', 'country']),  # supports both city and state level search
        ]
...
class SavedLead(models.Model):
    ...
    """
    user       = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='saved_leads')
...
class SearchCache(models.Model):
    ...
    """
    cache_key     = models.CharField(max_length=255, unique=True, db_index=True)  # leads:plumber:albany:new_york:US
