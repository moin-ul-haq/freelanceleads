...
class PlanSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            'id', 'name', 'price_usd',
            'search_limit', 'backlink_search_limit', 'ai_pitch_limit',
            'email_send_limit', 'saved_leads_limit', 'ai_chat_limit',
            'bulk_search_limit', 'features',
        )
...
class UsageCounterSerializer(serializers.ModelSerializer):
    ...
    def get_limit(self, obj):
        return self._get_limit(obj)

    def get_remaining(self, obj):
        limit = self._get_limit(obj)
        if limit == -1:
            return -1  # unlimited
